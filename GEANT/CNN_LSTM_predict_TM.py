...
class CNN_LSTM(nn.Module):
    ...
    def __init__(self):
        ...
        ############################### CNN ###################################
        self.conv1 = nn.Sequential(  # input size: (1, 23, 23)
            nn.Conv2d(
                in_channels=1,  # input height
                out_channels=16,  # output height
                kernel_size=3,  # filter size
                stride=1,  # filter moving step
                padding=2,  # padding
            ),  # output size: (16, 23, 23)
            nn.BatchNorm2d(16),
            nn.ReLU(),
            nn.Conv2d(16, 16, 5, 1, 2),
            nn.BatchNorm2d(16),
            nn.MaxPool2d(kernel_size=2, padding=1),  # output size: (16, 12, 12)
            nn.Dropout(),
        )
        ...
        # CNN to LSTM
        self.middle_out = nn.Linear(16 * 13 * 13, INPUT_SIZE)
        ############################### CNN ###################################
    ...
    def forward(self, x, batch_size):
        ...
        x = x.view(x.size(0), -1)  # (batch_size, 32, 6, 6) -> (batch_size, 32 * 6 * 6)
    ...
    def train(self):
        ...
        for i in range(train_len, len(x_data)):
            ...
            if USE_CUDA:
                ...
                # path = "E:/Tsinghua/master/Project/code/traffic matrix prediction/TM_result/GEANT/CNN_LSTM/CNN_LSTM_" + str(i - 2499) + ".txt"
